...
// الدالة السحرية لجلب البيانات من Supabase
    ...
    try {
        ...
        posts.forEach(post => {
            let imageHTML = post.image_url && post.image_url.trim() !== '' ? 
                `<div style="margin-top: 15px; text-align: center;"><img src="${post.image_url}" alt="${post.title}" style="max-width: 100%; height: auto; border-radius: 8px; border: 1px solid #333;" /></div>` : '';
...
window.addEventListener('DOMContentLoaded', () => {
    loadNewsTicker(); // تشغيل شريط الأخبار
    loadSectionContent('news'); // تحميل المحتوى الافتراضي
});
